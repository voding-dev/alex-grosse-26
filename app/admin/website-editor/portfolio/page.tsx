...
import { ChevronUp, ChevronDown, Plus, Eye, Trash2 } from "lucide-react";
...
export default function PortfolioEditorPage() {
  ...
  const handleMoveUp = async (index: number) => {
    ...
    await reorderPortfolioItems({ ids: newOrder.map((item) => item._id), email: adminEmail });
    toast({ title: "Order updated", description: "Portfolio item order has been updated." });
  };

  const handleMoveDown = async (index: number) => {
    if (index === portfolioItems.length - 1 || !adminEmail) return;
    const newOrder = [...portfolioItems];
  ...
  const handleDeleteConfirm = async () => {
    ...
    try {
      ...
      toast({ title: "Item deleted", description: `"${itemToDelete.title}" has been deleted.` });
  ...
  return (
    <div className="mx-auto max-w-7xl px-4 sm:px-6 py-8 sm:py-12">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
          <div>
            ...
            <p className="text-base sm:text-lg" style={{ color: '#666' }}>
              Manage portfolio items displayed on your homepage. Shows {portfolioItems.length} visible item{portfolioItems.length !== 1 ? 's' : ''}.
            </p>
          </div>
          <Button 
      ...
      <Card className="border mb-8" style={{ backgroundColor: 'rgba(88, 96, 52, 0.08)', borderColor: 'rgba(88, 96, 52, 0.2)' }}>
        <CardContent className="p-4 sm:p-6">
          <p className="text-sm" style={{ color: '#555' }}>
            <strong className="font-black uppercase tracking-wider" style={{ color: '#586034' }}>Portfolio Section</strong> - These items appear in the Portfolio section on your public homepage.
          </p>
        </CardContent>
      </Card>
      ...
      {portfolioItems.length === 0 ? (
        ...
        <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
          {portfolioItems.map((item, index) => (
            <Card key={item._id} className="group transition-all hover:shadow-lg relative" style={{ backgroundColor: '#fff', borderColor: 'rgba(0,0,0,0.1)' }}>
              <div className="cursor-pointer" onClick={() => router.push(`/admin/portfolio/${item._id}`)}>
                ...
                <CardContent>
                  <div className="flex items-center gap-3 flex-wrap">
                    <span className="text-xs font-bold uppercase tracking-wider flex items-center gap-1" style={{ color: '#586034' }}>
                      <Eye className="h-3 w-3" /> Visible on site
                    </span>
                    <span className="text-xs" style={{ color: '#888' }}>{item.categories.join(", ")}</span>
                  </div>
                </CardContent>
              </div>
              <div className="absolute bottom-4 right-4 z-10 flex items-center gap-2">
                <div className="flex flex-col gap-1">
                  ...
                  <Button variant="ghost" size="sm" onClick={(e) => { e.stopPropagation(); handleMoveDown(index); }} disabled={index === portfolioItems.length - 1} className="h-6 w-6 p-0 hover:bg-gray-100">
                    <ChevronDown className="h-3 w-3" style={{ color: '#555' }} />
                  </Button>
                </div>
                <Button variant="ghost" size="icon" onClick={(e) => { e.stopPropagation(); handleDeleteClick(item._id, item.title); }} className="hover:bg-red-50 rounded-full h-8 w-8" style={{ color: '#999' }}>
      ...
      {/* Draft Items */}
      {draftPortfolioItems.length > 0 && (
        <div className="mt-12">
          <h2 className="text-2xl font-black uppercase tracking-tight mb-6" style={{ fontWeight: '900', color: '#1a1a1a' }}>
            Draft Items ({draftPortfolioItems.length})
          </h2>
          <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
            {draftPortfolioItems.map((item) => (
              <Card key={item._id} className="group transition-all opacity-60 hover:opacity-80" style={{ backgroundColor: '#fff', borderColor: 'rgba(0,0,0,0.1)' }}>
                <div className="cursor-pointer" onClick={() => router.push(`/admin/portfolio/${item._id}`)}>
                  <CardHeader className="pb-3">
                    <CardTitle className="text-lg font-black uppercase tracking-wider mb-2" style={{ color: '#1a1a1a' }}>{item.title}</CardTitle>
                    <CardDescription style={{ color: '#666' }}>{item.clientName}</CardDescription>
                  </CardHeader>
                  <CardContent>
                    <span className="text-xs uppercase" style={{ color: '#888' }}>Status: {item.status}</span>
                  </CardContent>
                </div>
              </Card>
            ))}
      ...
      <Dialog open={deleteDialogOpen} onOpenChange={setDeleteDialogOpen}>
        <DialogContent className="max-w-md">
          <DialogHeader>
            <DialogTitle className="text-2xl font-black uppercase tracking-wider text-destructive">Delete Item</DialogTitle>
