...
// Get all window gallery images
export const list = query({
  handler: async (ctx) => {
    const images = await ctx.db
      .query("graphicDesignerWindowGallery")
...
// Add a new window gallery image
export const add = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    const now = Date.now();
    return await ctx.db.insert("graphicDesignerWindowGallery", {
...
// Remove window gallery image
export const remove = mutation({
  args: {
    id: v.id("graphicDesignerWindowGallery"),
    email: v.optional(v.string()),
  },
  handler: async (ctx, args) => {
...
// Reorder window gallery images
export const reorder = mutation({
  args: {
    ids: v.array(v.id("graphicDesignerWindowGallery")),
    email: v.optional(v.string()),
  },
  handler: async (ctx, args) => {
