...
// Get all design hero carousel images
...
// Add a new design hero carousel image
export const add = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    const now = Date.now();
    const heroId = await ctx.db.insert("designHeroCarousel", {
    ...
    await createOrUpdateMediaLibraryEntry(ctx, {
      ...
      filename: data.alt || "design-hero.jpg",
      type: "image",
      size: 0,
      alt: data.alt,
      displayLocation: {
...
// Update design hero carousel image
export const update = mutation({
  args: {
    id: v.id("designHeroCarousel"),
...
// Delete design hero carousel image
...
// Reorder design hero carousel images
export const reorder = mutation({
  args: {
    ids: v.array(v.id("designHeroCarousel")),
    email: v.optional(v.string()), // Dev mode: email for admin check
  },
  handler: async (ctx, args) => {
