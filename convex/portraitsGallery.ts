...
// Get all portraits gallery images
...
// Add a new portraits gallery image
export const add = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    const now = Date.now();
    const galleryId = await ctx.db.insert("portraitsGallery", {
    ...
    await createOrUpdateMediaLibraryEntry(ctx, {
      ...
      filename: data.alt || "portraits-gallery.jpg",
      type: "image",
      width: data.width,
      height: data.height,
      size: 0,
...
// Update portraits gallery image
export const update = mutation({
  args: {
    id: v.id("portraitsGallery"),
...
// Delete portraits gallery image
...
// Reorder portraits gallery images
export const reorder = mutation({
  args: {
    ids: v.array(v.id("portraitsGallery")),
    email: v.optional(v.string()), // Dev mode: email for admin check
  },
  handler: async (ctx, args) => {
