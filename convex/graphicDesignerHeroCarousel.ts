...
// Get all graphic designer hero carousel images
...
// Add a new graphic designer hero carousel image
export const add = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    const now = Date.now();
    const heroId = await ctx.db.insert("graphicDesignerHeroCarousel", {
    ...
    await createOrUpdateMediaLibraryEntry(ctx, {
      ...
      filename: data.alt || "graphic-designer-hero.jpg",
      type: "image",
      size: 0,
      alt: data.alt,
      displayLocation: {
        type: "portfolio",
        entityId: "graphic-designer",
        entityName: "Graphic Designer",
      },
    });
...
// Update graphic designer hero carousel image
export const update = mutation({
  args: {
    id: v.id("graphicDesignerHeroCarousel"),
...
// Delete graphic designer hero carousel image
...
// Reorder graphic designer hero carousel images
export const reorder = mutation({
  args: {
    ids: v.array(v.id("graphicDesignerHeroCarousel")),
    email: v.optional(v.string()), // Dev mode: email for admin check
  },
  handler: async (ctx, args) => {
